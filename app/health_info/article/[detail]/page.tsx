...
export default function ArticleDetail({ params: { detail } }: Props) {
  const id_str: string = detail.replace('article_', '').replace('_detail', '');
  const googleCrmp = detail.replace('article_', 'Article').replace('_detail', 'Detail');
  const twStayles = tv({
    variants: {
      style: {
        container:
  ...
  return (
    <>
      <main className={twStayles({ style: 'container' })}>
        ...
        { name: googleCrmp, href: `/health_info/prevent/${detail}` },
      ]} />
    </>
  );
}